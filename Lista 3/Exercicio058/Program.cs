...
class Program
{
    static void Main()
    {
        ...
        List<Conta> contas = new List<Conta>
        {
            ...
            new ContaCorrente("200-2", titular, 500.00m, 200.00m),
            new ContaCorrenteEspecialCorporate("400-4", titular, 10000.00m, 5000.00m, 100m, "12.345.678/0001-90")
        };

        foreach (Conta item in contas)
        {
            Console.WriteLine($"--- Analisando {item.GetType().Name} ---");

            if (item is ContaCorrenteEspecialCorporate corp)
            {
                Console.WriteLine($"[OPERADOR IS] Identificada Conta Corporate. CNPJ: {corp.CnpjEmpresa}");
            }

            ContaCorrente cc = item as ContaCorrente;
            if (cc != null)
            {
                Console.WriteLine($"[OPERADOR AS] Cast bem-sucedido para ContaCorrente. Limite disponível: R$ {cc.Limite:F2}");
            }
            else
            {
                Console.WriteLine("[OPERADOR AS] Não foi possível converter para ContaCorrente (retornou null).");
            }
