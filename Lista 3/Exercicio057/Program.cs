...
class Program
{
    static void Main()
    {
        ...
        Conta objNivel4 = new ContaCorrenteEspecialCorporate("400-4", titular, 10000.00m, 5000.00m, 100m, "12.345.678/0001-90");

        Console.WriteLine("=== UPCASTING (IMPLÍCITO) ===");
        Conta objNivel1 = objNivel4;
        Console.WriteLine($"Atribuído com sucesso para referência base: {objNivel1.GetType().Name}");

        Console.WriteLine("\n=== DOWNCASTING EXPLÍCITO (SUCESSO) ===");
        ContaCorrenteEspecialCorporate objRecuperado = (ContaCorrenteEspecialCorporate)objNivel1;
        Console.WriteLine($"Acessando propriedade do nível 4 via Downcasting: CNPJ = {objRecuperado.CnpjEmpresa}");

        Console.WriteLine("\n=== DOWNCASTING INVÁLIDO (TRATADO COM TRY/CATCH) ===");
        Conta objApenasBase = new Conta("100-1", titular, 1000.00m);

        try
        {
            ContaCorrente cCorrenteInvalida = (ContaCorrente)objApenasBase;
        }
        catch (InvalidCastException ex)
        {
            Console.WriteLine($"[ERRO DE CASTING] Não foi possível realizar o cast explícito: {ex.Message}");
        }
    }
}