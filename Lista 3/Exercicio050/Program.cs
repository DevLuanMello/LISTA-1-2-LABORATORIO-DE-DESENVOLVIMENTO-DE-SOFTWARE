...
public class Funcionario : Pessoa
{
    ...
    public void Promover(string novoCargo, decimal percentualAumento)
    {
        _cargo = novoCargo;
        _salario += _salario * (percentualAumento / 100m);
        Console.WriteLine($"[PROMOÇÃO] {_nome} foi promovido a {_cargo}! Novo salário: {_salario:C2}");
    }
...
public class Gerente : Funcionario
{
    ...
    public void AjustarBonusEAniversario(decimal novoBonus)
    {
        _bonus = novoBonus;
        _idade++;
        Console.WriteLine($"[ATUALIZAÇÃO DE GERENTE] {_nome} completou {_idade} anos. Novo bônus: {_bonus:C2}. Salário base: {_salario:C2}");
    }

    public override void Apresentar()
    {
        Console.WriteLine($"Sou {_nome}, gerencio a equipe aos {_idade} anos. Salário base: {_salario:C2} | Bônus: {_bonus:C2}.");
    }
...
class Program
{
    static void Main()
    {
        Funcionario dev = new Funcionario("Mariana", 32, "Desenvolvedora C#", 7500.00m);
        Gerente gerente = new Gerente("Roberto", 45, 12000.00m, 3500.00m);

        dev.Apresentar();
        dev.Promover("Desenvolvedora Senior", 20m);

        Console.WriteLine(new string('-', 50));

        gerente.Apresentar();
        gerente.AjustarBonusEAniversario(5000.00m);
    }
}